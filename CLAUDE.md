...
| **Objective custom data** | Optional structured per-instance progress state for custom `QuestObjectiveType`s, held as `Map<String,Object>` on `QuestObjectiveInstance.getCustomData()` and persisted as a JSON `custom_data` column on the `QuestObjectiveInstanceDAO` table. Used for set-like objectives (distinct biomes, one-of-each). Mutate in place + `markCustomDataDirty()`. |
