...
/**
 * Built-in objective type for tracking block place progress.
 * <p>
 ...
 * names (e.g. {@code COBBLESTONE}) or custom block identifiers from supported plugins.
 * If the list is empty, any block placement counts toward progress.
 */
public class BlockPlaceObjectiveType implements QuestObjectiveType {

    public static final NamespacedKey KEY = new NamespacedKey(McRPGMethods.getMcRPGNamespace(), "block_place");
    ...
    private BlockPlaceObjectiveType(@NotNull Set<CustomBlockWrapper> validBlocks) {
    ...
    @NotNull
    @Override
    public BlockPlaceObjectiveType parseConfig(@NotNull Section section) {
    ...
    public long processProgress(@NotNull QuestObjectiveInstance instance,
        ...
        CustomBlockWrapper placedBlock = new CustomBlockWrapper(blockContext.getBlockPlaceEvent().getBlock());
        return validBlocks.contains(placedBlock) ? 1 : 0;
    }
    ...
    public String describeObjective(@NotNull McRPGPlayer player, long requiredProgress) {
        ...
        if (validBlocks.isEmpty()) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_BLOCK_PLACE_ANY,
                    Map.of("count", count));
        }
        if (validBlocks.size() == 1) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_BLOCK_PLACE_SINGLE,
        ...
        StringBuilder sb = new StringBuilder(localization.getLocalizedMessage(player,
                LocalizationKey.QUEST_OBJECTIVE_BLOCK_PLACE_MULTI_HEADER, Map.of("count", count)));
        for (CustomBlockWrapper block : validBlocks) {
            sb.append("\n").append(localization.getLocalizedMessage(player,
                    LocalizationKey.QUEST_OBJECTIVE_BLOCK_PLACE_MULTI_ITEM, Map.of("block", block.blockName())));
        }
        return sb.toString();
    }
