...
/**
 * Built-in objective type for tracking anvil repair progress.
 * <p>
 ...
 * names (e.g. {@code DIAMOND_PICKAXE}) or custom item identifiers from supported plugins.
 * If the list is empty, any anvil repair counts toward progress.
 */
public class AnvilRepairObjectiveType implements QuestObjectiveType {

    public static final NamespacedKey KEY = new NamespacedKey(McRPGMethods.getMcRPGNamespace(), "anvil_repair");
     ...
     */
    public AnvilRepairObjectiveType() {
     ...
     */
    private AnvilRepairObjectiveType(@NotNull Set<CustomItemWrapper> validItems) {
     ...
     * {@inheritDoc}
     *
     * @return the namespaced key {@code mcrpg:anvil_repair}
     */
    @NotNull
    @Override
    ...
    @NotNull
    @Override
    public AnvilRepairObjectiveType parseConfig(@NotNull Section section) {
        ...
        return new AnvilRepairObjectiveType(items);
    }
     ...
     * @return {@code true} if the context is a {@link AnvilRepairQuestContext}
     */
    @Override
    public boolean canProcess(@NotNull QuestObjectiveProgressContext context) {
        return context instanceof AnvilRepairQuestContext;
    }

    /**
     * {@inheritDoc}
     * <p>
     * Each anvil repair operation counts as 1 progress. The result item from the
     * {@link org.bukkit.event.inventory.InventoryClickEvent} is matched against the
     * configured valid items set.
     *
     * @param instance the objective instance to potentially progress
     * @param context  the progress context from an anvil repair event
    ...
    public long processProgress(@NotNull QuestObjectiveInstance instance,
        ...
        if (!(context instanceof AnvilRepairQuestContext anvilContext)) {
            return 0;
        }
        ...
        var event = anvilContext.getInventoryClickEvent();
     ...
     * @return a descriptive string for anvil repair objectives
     */
    @NotNull
    @Override
    public String describeObjective(@NotNull McRPGPlayer player, long requiredProgress) {
        ...
        if (validItems.isEmpty()) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_ANVIL_REPAIR_ANY,
                    Map.of("count", count));
        }
        if (validItems.size() == 1) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_ANVIL_REPAIR_SINGLE,
        ...
        StringBuilder sb = new StringBuilder(localization.getLocalizedMessage(player,
                LocalizationKey.QUEST_OBJECTIVE_ANVIL_REPAIR_MULTI_HEADER, Map.of("count", count)));
        for (CustomItemWrapper item : validItems) {
            sb.append("\n").append(localization.getLocalizedMessage(player,
                    LocalizationKey.QUEST_OBJECTIVE_ANVIL_REPAIR_MULTI_ITEM, Map.of("item", resolveItemDisplayName(item))));
        }
        return sb.toString();
    }
