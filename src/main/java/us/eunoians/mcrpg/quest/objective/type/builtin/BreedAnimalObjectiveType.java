...
/**
 * Built-in objective type for tracking animal breeding progress.
 * <p>
 ...
 * names (e.g. {@code COW}) or custom entity identifiers from supported plugins. If the list
 * is empty, any animal breeding counts toward progress.
 */
public class BreedAnimalObjectiveType implements QuestObjectiveType {

    public static final NamespacedKey KEY = new NamespacedKey(McRPGMethods.getMcRPGNamespace(), "breed_animal");
     ...
     */
    public BreedAnimalObjectiveType() {
        this.validEntities = Set.of();
    }

    private BreedAnimalObjectiveType(@NotNull Set<CustomEntityWrapper> validEntities) {
    ...
    @NotNull
    @Override
    public BreedAnimalObjectiveType parseConfig(@NotNull Section section) {
    ...
    public long processProgress(@NotNull QuestObjectiveInstance instance,
        ...
        return validEntities.contains(breedContext.getChildWrapper()) ? 1 : 0;
    }
    ...
    public String describeObjective(@NotNull McRPGPlayer player, long requiredProgress) {
        ...
        if (validEntities.isEmpty()) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_BREED_ANIMAL_ANY,
                    Map.of("count", count));
        }
        if (validEntities.size() == 1) {
            return localization.getLocalizedMessage(player, LocalizationKey.QUEST_OBJECTIVE_BREED_ANIMAL_SINGLE,
        ...
        StringBuilder sb = new StringBuilder(localization.getLocalizedMessage(player,
                LocalizationKey.QUEST_OBJECTIVE_BREED_ANIMAL_MULTI_HEADER, Map.of("count", count)));
        for (CustomEntityWrapper entity : validEntities) {
            sb.append("\n").append(localization.getLocalizedMessage(player,
                    LocalizationKey.QUEST_OBJECTIVE_BREED_ANIMAL_MULTI_ITEM, Map.of("entity", entity.entityName())));
        }
        return sb.toString();
    }
