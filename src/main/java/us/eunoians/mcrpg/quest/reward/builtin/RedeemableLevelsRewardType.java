...
/**
 * Built-in reward type that grants redeemable skill levels to a player's
 * {@link us.eunoians.mcrpg.entity.player.PlayerExperienceExtras} pool.
 * Redeemable levels can be applied to specific skills by the player.
 * <p>
 * Config format:
 * <pre>
 ...
 * </pre>
 */
public class RedeemableLevelsRewardType implements QuestRewardType {

    public static final NamespacedKey KEY = new NamespacedKey(McRPGMethods.getMcRPGNamespace(), "redeemable_levels");
     ...
     */
    public RedeemableLevelsRewardType() {
        this.amount = 0;
        this.localizationRoute = null;
        this.displayLabel = "";
    }

    private RedeemableLevelsRewardType(int amount, @Nullable Route localizationRoute, @NotNull String displayLabel) {
        this.amount = amount;
    ...
    @NotNull
    @Override
    public RedeemableLevelsRewardType parseConfig(@NotNull Section section) {
        ...
        return new RedeemableLevelsRewardType(amount, null, "");
    }
    ...
    public void grant(@NotNull Player player) {
        ...
        mcRPGPlayerOpt.ifPresent(p -> p.getExperienceExtras().modifyRedeemableLevels(amount));
    }
    ...
    @NotNull
    @Override
    public RedeemableLevelsRewardType fromSerializedConfig(@NotNull Map<String, Object> config) {
    ...
    @NotNull
    @Override
    public RedeemableLevelsRewardType withLocalizationRoute(@NotNull Route route) {
        return new RedeemableLevelsRewardType(amount, route, displayLabel);
    }

    @NotNull
    @Override
    public RedeemableLevelsRewardType withInlineDisplayLabel(@NotNull String label) {
        return new RedeemableLevelsRewardType(amount, localizationRoute, label);
    }

    @NotNull
    @Override
    public String describeForDisplay() {
        return amount + " Redeemable Level(s)";
    }
    ...
    public String describeForDisplay(@NotNull McRPGPlayer player) {
        ...
        if (localizationRoute != null) {
            try {
                ...
                McRPG.getInstance().getLogger().log(Level.WARNING,
                        "Failed to resolve display label for redeemable_levels reward", e);
            }
        }
        if (!displayLabel.isEmpty()) {
        ...
        try {
            String label = localization.getLocalizedMessage(player, LocalizationKey.QUEST_REWARD_REDEEMABLE_LEVELS_FORMAT, vars);
            return prependDefaultColor(localization, player, label);
        } catch (Exception e) {
            McRPG.getInstance().getLogger().log(Level.WARNING,
                    "Failed to resolve display label for redeemable_levels reward", e);
