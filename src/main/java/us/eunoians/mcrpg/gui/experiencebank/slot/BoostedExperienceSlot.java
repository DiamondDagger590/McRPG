 ...
 * the player's boosted experience count and explain how it works.
 */
public final class BoostedExperienceSlot implements McRPGSlot {

    @NotNull
    @Override
    public ItemBuilder getItem(@NotNull McRPGPlayer mcRPGPlayer) {
        var localizationManager = McRPG.getInstance().registryAccess().registry(RegistryKey.MANAGER).manager(McRPGManagerKey.LOCALIZATION);
        ItemBuilder itemBuilder = ItemBuilder.from(localizationManager.getLocalizedSection(mcRPGPlayer, LocalizationKey.EXPERIENCE_BANK_GUI_BOOSTED_EXPERIENCE_SLOT_DISPLAY_ITEM))
    ...
    private Map<String, String> getPlaceholders(@NotNull McRPGPlayer mcRPGPlayer) {
        ...
        double boostedExperienceUsageRate = configFile.getDouble(MainConfigFile.BOOSTED_EXPERIENCE_USAGE_RATE);
        ...
        int baseExampleExperience = Integer.parseInt(localizationManager
                .getLocalizedMessage(mcRPGPlayer, LocalizationKey.EXPERIENCE_BANK_GUI_BOOSTED_EXPERIENCE_SLOT_EXAMPLE_BASE_AMOUNT));
        ...
        placeholders.put("boosted-experience-usage-rate", localizationManager.getDisplayDecimalFormatter().formatDisplayDecimal(mcRPGPlayer, boostedExperienceUsageRate));
        placeholders.put("boosted-experience", Integer.toString(mcRPGPlayer.getExperienceExtras().getBoostedExperience()));
        return placeholders;
    }
}
