	...
	buildGUIFunction = (GUIBuilder builder) -> {
	  FileConfiguration config = McRPG.getInstance().getFileManager().getFile(FileManager.Files.fromString(abiltyToReplace.getGenericAbility().getSkill()));
		...
		Inventory inv = Bukkit.createInventory(null, guiConfig.getInt("Size"),
		  Methods.color(guiConfig.getString("Title").replace("%OldAbility", abiltyToReplace.getGenericAbility().getName())
	  ...
	  confirmMeta.setDisplayName(Methods.color(guiConfig.getString("ConfirmItem.DisplayName")));
	  ...
	  denyMeta.setDisplayName(Methods.color(guiConfig.getString("DenyItem.DisplayName")));
