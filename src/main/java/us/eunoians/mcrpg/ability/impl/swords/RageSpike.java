...
public final class RageSpike extends McRPGAbility implements ConfigurableActiveAbility, ConfigurableSkillAbility, ComboActivatable {
    ...
    /**
     * Gets the velocity to apply to the player and enemies for the given tier.
     *
     * @param tier The tier to get the velocity for.
     * @return The velocity for the given tier.
     */
    public int getVelocity(int tier) {
        ...
        if (swordsConfig.contains(tierRoute)) {
            ...
            parser = new Parser(swordsConfig.getString(allTiersRoute, "5"));
        }
        parser.setVariable("tier", tier);
