...
public class HerbalismMethods {
  ...
  public static HashSet<Block> findChorusPlant(Block target) {
    return findChorusPlant(target, new HashSet<>());
  }
  ...
  public static int calculateMultiBlockPlantDrops(BlockState blockState) {
    ...
    int dropAmount = McRPG.getPlaceStore().isTrue(block) ? 0 : 1;
    ...
    if(blockType == Material.CHORUS_PLANT) {
      ...
      for(int y = 1; y < 3; y++) {
        ...
        if(McRPG.getPlaceStore().isTrue(relativeBlock)) {
          McRPG.getPlaceStore().setFalse(relativeBlock);
        }
        else {
          dropAmount++;
        }
