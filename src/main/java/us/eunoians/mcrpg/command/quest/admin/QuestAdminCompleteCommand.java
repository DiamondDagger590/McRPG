...
/**
 * Command: {@code /mcrpg quest admin complete <quest_uuid>}
 * <p>
 * Force-completes an active quest instance. The normal lifecycle fires
 * (rewards granted, events dispatched, quest retired to cache).
 */
public class QuestAdminCompleteCommand extends QuestCommandBase {

    private static final Permission QUEST_ADMIN_COMPLETE_PERMISSION = Permission.of("mcrpg.quest.admin.complete");
    ...
    public static void registerCommand() {
        ...
        commandManager.command(commandManager.commandBuilder("mcrpg")
                ...
                .required("quest_uuid", StringParser.stringParser(),
                        RichDescription.richDescription(mm.deserialize("<gray>The UUID of the quest to force-complete")))
                .permission(Permission.anyOf(ROOT_PERMISSION, QUEST_ADMIN_PERMISSION, QUEST_ADMIN_COMPLETE_PERMISSION))
                .handler(commandContext -> {
                    ...
                    if (target.getQuestState() != QuestState.IN_PROGRESS) {
                        sender.sendMessage(localizationManager.getLocalizedMessageAsComponent(sender,
                                LocalizationKey.QUEST_ADMIN_COMPLETE_NOT_IN_PROGRESS,
                                Map.of(QUEST_STATE.getPlaceholder(), target.getQuestState().toString())));
                        return;
                    }

                    QuestInstance finalTarget = target;
                    questManager.getQuestDefinition(target.getQuestKey()).ifPresentOrElse(
                            definition -> {
                                finalTarget.complete(definition);
                                sender.sendMessage(localizationManager.getLocalizedMessageAsComponent(sender,
                                        LocalizationKey.QUEST_ADMIN_COMPLETE_SUCCESS,
                                        Map.of(QUEST_KEY.getPlaceholder(), finalTarget.getQuestKey().toString(),
                                                QUEST_UUID.getPlaceholder(), questUUID.toString())));
                            },
                            () -> sender.sendMessage(localizationManager.getLocalizedMessageAsComponent(sender,
                                    LocalizationKey.QUEST_ADMIN_COMPLETE_DEFINITION_NOT_FOUND,
                                    Map.of(QUEST_KEY.getPlaceholder(), finalTarget.getQuestKey().toString())))
                    );
                }));
    }
