...
public final class MobHealthbarUtils {
  ...
  public static void handleMobHealthbars(Player p, LivingEntity target, double damage) {
	if (Mcmmox.getInstance().isHealthBarPluginEnabled()  || !Mcmmox.getInstance().getConfig().getBoolean("Configuration.HealthBarEnabled")) {
	  return;
	}
...
//TODO Player settings
	...
	int displayTime = Mcmmox.getInstance().getConfig().getInt("Configuration.HealthBarDisplayTime");

	if (displayTime != -1) {
	  boolean updateName = !ChatColor.stripColor(oldName).equalsIgnoreCase(ChatColor.stripColor(newName));

	  if (updateName) {
		target.setMetadata(Mcmmox.getInstance().getCustomNameKey(), new FixedMetadataValue(Mcmmox.getInstance(), oldName));
		target.setMetadata(Mcmmox.getInstance().getCustomVisibleKey(), new FixedMetadataValue(Mcmmox.getInstance(), oldNameVisible));
	  }
	  else if (!target.hasMetadata(Mcmmox.getInstance().getCustomNameKey())) {
		target.setMetadata(Mcmmox.getInstance().getCustomNameKey(), new FixedMetadataValue(Mcmmox.getInstance(), ""));
		target.setMetadata(Mcmmox.getInstance().getCustomVisibleKey(), new FixedMetadataValue(Mcmmox.getInstance(), false));
	  }

	  new MobHealthDisplayUpdaterTask(target).runTaskLater(Mcmmox.getInstance(), displayTime * 20); // Clear health display after 3 seconds
	}
  }
