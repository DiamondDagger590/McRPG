...
public class HashChunkletManager implements ChunkletManager {
  ...
  public void unloadChunk(int cx, int cz, World world){
	...
	for(int y = 0; y < 4; y++){
	  unload(cx, cz, world, dataDir, y);
	}
  }

  private void unload(int cx, int cz, World world, File dataDir, int y){
  ...
  @Override
  public void saveAll(){
	for(World world : Mcmmox.getInstance().getServer().getWorlds()){
	  saveWorld(world);
	}
  }
  ...
  public void cleanUp(){
	for(String key : store.keySet()){
	  if(store.get(key).isEmpty()){
		String[] info = key.split(",");
		File dataDir = new File(Mcmmox.getInstance().getServer().getWorld(info[0]).getWorldFolder(), "mcmmo_data");
