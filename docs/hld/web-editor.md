...
| US-3 | As a server owner, I want to create a new quest using a step-by-step wizard | Wizard guides through: basics → objectives → rewards → review → save |
...
| US-6 | As a server owner, I want to see a preview of how my quest looks in the Minecraft GUI | Editor renders an approximation of the quest board slot, detail view, and reward display |
| US-7 | As a server owner, I want to apply my changes and have them take effect on my server | Editor sends changes to plugin; plugin writes YAML and prompts me to run `/mcrpg quest admin reload` |
...
| ID | Story | Acceptance Criteria |
|----|-------|-------------------|
| US-9 | As a server owner, I want to edit board rarity tiers (weights, multipliers, display settings) | Visual form with sliders for weights, numeric inputs for multipliers, material picker for display |
| US-10 | As a server owner, I want to edit board categories (slot counts, visibility, refresh types) | Form shows all category fields with dropdowns for enums and numeric inputs for bounds |
| US-11 | As a server owner, I want to create or delete board categories | "Add Category" and "Delete" buttons with confirmation dialogs |
| US-12 | As a server owner, I want to edit rotation schedule (time, timezone, weekly reset day) | Timezone picker, time input, day-of-week dropdown |

### Board Simulator (Phase 1.5)

| ID | Story | Acceptance Criteria |
|----|-------|-------------------|
| US-13 | As a server owner, I want to simulate N board rolls and see how often each quest appears | Simulator tab with roll count input; results show quest frequency distribution |
| US-14 | As a server owner, I want to simulate rolls for a specific player profile (permissions, land membership) | Player profile builder with permission checkboxes; simulator uses profile for filtering |
| US-15 | As a server owner, I want to see rarity distribution statistics across simulated rolls | Bar chart or table showing actual vs. expected rarity distribution |

### Quest Templates (Phase 2 — Future)

| ID | Story | Acceptance Criteria |
|----|-------|-------------------|
| US-16 | As a server owner, I want to create quest templates using a visual builder | Drag-and-drop or form-based variable, phase, and objective creation |
| US-17 | As a server owner, I want to preview generated quests from a template at each rarity tier | "Preview" button generates sample quests and shows them side-by-side by rarity |
...
13. Admin clicks "Apply Changes"
14. Web editor compresses the change payload and uploads it to bytebin → receives a `changeKey`
15. Web editor sends the `changeKey` to the plugin via the bytesocks channel (signed message)
16. Plugin downloads the change payload from bytebin, validates it, and writes updated YAML files to disk
17. Plugin sends a confirmation back through bytesocks
18. Web editor shows: "Changes saved! Run `/mcrpg quest admin reload` to apply."
19. Plugin releases the session lock when the admin closes the editor or the session times out
...
Creating a new quest uses a multi-step wizard (US-3) to avoid overwhelming the user:

**Step 1 — Basics:**
- "What should this quest be called?" → text input for display name
- "Who can do this quest?" → dropdown (Single Player / Land Group / etc.)
- "Does this quest expire?" → toggle + duration input
- "Can this quest be repeated?" → dropdown (Once / Repeatable / Cooldown / etc.)

**Step 2 — Objectives:**
- "What do players need to do?" → [+ Add Objective] button
- Each objective: type dropdown → dynamic form based on type
  - "Break Blocks" → block type picker + count input
  - "Kill Mobs" → entity type picker + count input
  - etc.
- "Should objectives be completed in order or all at once?" → ALL / ANY toggle

**Step 3 — Rewards:**
- "What do players earn?" → [+ Add Reward] button
- Each reward: type dropdown → dynamic form
...
**Step 6 — Review:**
- Summary of all choices + in-game preview
- [Create Quest] button
...
5. Implement manifest store and common picker components (MaterialPicker, EntityTypePicker, etc.)
6. Implement quest store with change tracking
7. Implement QuestList, QuestEditor, and all sub-editors (Phase, Stage, Objective, Reward)
8. Implement QuestWizard (new quest creation flow)
9. Implement FilePicker for YAML file targeting
10. Implement QuestPreview (in-game GUI approximation)
11. Implement board config store and editors (RarityEditor, CategoryEditor, RotationEditor)
12. Implement validation layer
13. Implement "Apply Changes" flow (compress → upload → notify → confirm)
14. Implement "Export as YAML" emergency export
15. Implement error handling and reconnection logic
16. Implement ConnectionStatus indicator
...
**Goal:** Server owners can simulate quest board rolls to validate their configuration.

**Frontend work:**
1. Port `SlotGenerationLogic` and `QuestPool` weighted selection to TypeScript
2. Implement `PlayerProfileBuilder` component
3. Implement `BoardSimulator` runner
4. Implement `SimulationResults` with frequency tables and distribution charts
5. Validate TypeScript simulation output against Java implementation for correctness
